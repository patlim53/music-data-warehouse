...
--
-- This script assumes the 'staging_tables_setup.sql' has been
-- run successfully at least once and its indexes are built.
--
-- This script can be run multiple times safely.
-- ========================================================

-- ----------------------------------------------------------------
-- STEP 0: Create Database, Set Timeouts, and Create Tables
-- ----------------------------------------------------------------
...
-- ⚙️ INCREASE TIMEOUTS FOR LARGE LOADS ⚙️
-- This prevents the 'Lost connection' error during long queries.
SET SESSION net_read_timeout = 3600;
SET SESSION net_write_timeout = 3600;

-- ⚙️ DROP TABLES IN THE CORRECT ORDER ⚙️
-- Drop the fact table FIRST (as it depends on the dimensions)
DROP TABLE IF EXISTS fact_song_performance;
-- Now it's safe to drop the dimension tables
...
CREATE TABLE fact_song_performance (
...
-- Add a placeholder date for non-Grammy, weekly data
...
-- ----------------------------------------------------------------
-- STEP 2.5: ADD WAREHOUSE DIMENSION INDEXES
-- ----------------------------------------------------------------
-- Add indexes to the dimension tables' JOIN columns.
-- This is CRITICAL for fact table load performance.

CREATE INDEX idx_song_track_name ON dim_song (track_name);
...
-- Clear the fact table to avoid duplicate entries on re-runs.
...
SELECT
    ...
    a.artist_id,
    p.platform_id,
    20230101 AS date_id, -- Placeholder date
    st.chart_rank,
    st.peak_rank,
    st.previous_rank,
    st.weeks_on_chart,
...
SELECT
    ...
    p.platform_id,
    d.date_id,
    -- 🚀 FIX: Use LIKE to catch 'True' even with hidden characters
    CASE
        WHEN g.is_winner LIKE '%True%' THEN 1
        WHEN g.is_winner LIKE '%False%' THEN 0
        ELSE NULL
    END AS is_winner
FROM music_source.stage_grammy g
LEFT JOIN dim_song s ON g.song_album_name = s.track_name
LEFT JOIN dim_artist a ON g.artist_name = a.artist_name
JOIN dim_platform p ON p.platform_name = 'Grammy'
JOIN dim_date d ON g.year = d.year
-- Only insert rows where we can match both artist and song
